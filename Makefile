...
deploy:
	...
	sam deploy --no-progressbar --s3-bucket hendry-lambdas --s3-prefix protect --stack-name $(STACK) --parameter-overrides DomainName=$(DOMAINNAME) ACMCertificateArn=$(ACMCERTIFICATEARN) PushoverToken=$(PUSHOVER_TOKEN) PushoverUserKey=$(PUSHOVER_USER_KEY) --no-confirm-changeset --no-fail-on-empty-changeset --capabilities CAPABILITY_IAM --disable-rollback

build-MainFunction:
	CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -ldflags="-X main.gitCommit=$(GIT_COMMIT)" -o ${ARTIFACTS_DIR}/bootstrap
